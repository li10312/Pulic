...
class ResLayer(Sequential):
    ...
    def __init__(self,
                 ...
                 inplanes: int,
                 planes: int,
                 num_blocks: int,  # 当前stage有几个残差块 50:(Bottleneck,(3,4,6,3))
                 stride: int = 1,  # 下采样倍数，在第一个block进行下采样
                 avg_down: bool = False,  # 决定卷积层的方案，使不使用AvgPool
                 conv_cfg: OptConfigType = None,
                 norm_cfg: ConfigType = dict(type='BN'),
                 downsample_first: bool = True,  # 第一个block是否进行下采样
                 # heads=4,  # new
                 # mhsa=False,  # new
                 **kwargs) -> None:
        self.block = block
        ...
        downsample = None
        if stride != 1 or inplanes != planes * block.expansion:  # ResNet50->152第3->5残差block第一层需要下采样
            # stride!=1表示要做下采样，inplanes != planes * block.expansion表示要做升降维。
            downsample = []
            conv_stride = stride
            if avg_down:  # 使用ResNetV1d用avgpool来做下采样
                conv_stride = 1
                downsample.append(
                    nn.AvgPool2d(
                        kernel_size=stride,
        ...
        layers = []
        if downsample_first:   # 搭建每一块残差block的第一层
            layers.append(
            ...
            for _ in range(1, num_blocks):  # 当前stage的剩余残差块堆叠起来
                layers.append(
                    block(
                        inplanes=inplanes,
                        planes=planes,
